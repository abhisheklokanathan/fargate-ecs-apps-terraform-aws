...
resource "aws_ecs_service" "ecs_service" {
  ...
  cluster         = data.terraform_remote_state.platform_demo.outputs.ecs_cluster_name
  launch_type     = "FARGATE"

  network_configuration {
    # subnets           = [data.terraform_remote_state.platform.outputs.ecs_public_subnets]
    subnets = [
          data.terraform_remote_state.platform.outputs.public_1_subnet_cidr,
          data.terraform_remote_state.platform.outputs.public_2_subnet_cidr,
          data.terraform_remote_state.platform.outputs.public_3_subnet_cidr
          ]
...
resource "aws_alb_listener_rule" "ecs_alb_listener_rule" {
  listener_arn = data.terraform_remote_state.platform_demo.outputs.ecs_alb_listener_arn
  priority     = 100
